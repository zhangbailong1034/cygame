// Level generation script for 100 idiom puzzle levels
const fs = require('fs');
const path = require('path');

// === Idiom Pool with meanings ===
const idiomPool = [
  // --- Easy / Common idioms ---
  ...
  { answer: '朝气蓬勃', meaning: '形容充满了生机和活力。' },
...
// Pick random distractors not in used chars
const allChars = '天地人风云雨雪冰霜山水木石金火土日月星斗江河湖海龙虎凤鹰马牛羊鱼鸟花草木竹林田禾米谷城乡道路门窗户院子春夏秋冬早晚朝夕今古东西南北前后左右上下中正大小多少远近高低长短轻重快慢明暗冷热红白青绿黄蓝紫黑白赤橙红绿蓝黑白';

function pickDistractors(usedChars, count) {
  const available = allChars.split('').filter(c => !usedChars.includes(c));
 ...
 * @param {Array} idioms - array of {answer, meaning}
 * @param {number} numHorizontal - how many of the idioms are horizontal
 * @param {number} numVertical - how many are vertical (placed in extra columns)
 * @param {number} totalCols - total grid columns (4 = tight, 5 = has gap column)
 */
function buildLevel(levelId, difficulty, idioms, numHorizontal, numVertical, totalCols) {
  const totalIdioms = numHorizontal + numVertical;
  const configuredIdioms = [];

  // Place horizontal idioms on separate rows (one per row)
  for (let i = 0; i < numHorizontal; i++) {
    const answer = idioms[i].answer;
    const row = i;
    configuredIdioms.push({
      direction: 'horizontal',
      row,
      startCol: 0,
      endCol: answer.length - 1,  // 3 for 4-char idioms
      answer,
      meaning: idioms[i].meaning,
    });
  }

  // Place vertical idioms in the "gap" column(s) starting at col 4
  // Vertical idioms need 4 consecutive rows
  for (let i = 0; i < numVertical; i++) {
    const answer = idioms[numHorizontal + i].answer;
    const col = 4 + i;  // col 4, 5, etc. (must be >= answer length = 4)
    configuredIdioms.push({
      direction: 'vertical',
      startRow: 0,
      endRow: answer.length - 1,  // 3 for 4-char idioms
      col,
      answer,
      meaning: idioms[numHorizontal + i].meaning,
    });
  }
  ...
  for (const idiom of configuredIdioms) {
  ...
  // At minimum, horizontal idioms define rows. For vertical-only levels, use endRow + 1.
  const rows = Math.max(maxRow + 1, numHorizontal);
  // Columns: max(endCol for horizontals, col for verticals) + 1, but at least 4
  const cols = Math.max(maxCol + 1, totalCols);

  // Pick fixed cell: first character of first idiom
  const firstIdiom = configuredIdioms[0];
  let fixedRow, fixedCol, fixedChar;
  if (firstIdiom.direction === 'horizontal') {
    fixedRow = firstIdiom.row;
    fixedCol = firstIdiom.startCol;
    fixedChar = firstIdiom.answer[0];
  } else {
    fixedRow = firstIdiom.startRow;
    fixedCol = firstIdiom.col;
    fixedChar = firstIdiom.answer[0];
  }
  ...
  for (const idiom of configuredIdioms) {
    ...
    if (idiom.direction === 'horizontal') {
      for (let ci = 0; ci < chars.length; ci++) {
        if (idiom.row === fixedRow && idiom.startCol + ci === fixedCol) continue;
        fragments.push({
          text: chars[ci],
          length: 1,
      ...
      for (let ci = 0; ci < chars.length; ci++) {
        if (idiom.startRow + ci === fixedRow && idiom.col === fixedCol) continue;
        fragments.push({
          text: chars[ci],
          length: 1,
  ...
  const numDistractors = difficulty === 'easy' ? 2 : difficulty === 'medium' ? 3 : difficulty === 'hard' ? 4 : 5;
  const distractors = pickDistractors([...usedChars], numDistractors);

  const totalFrags = fragments.length;
  const minScore = difficulty === 'easy' ? totalFrags * 15 :
                   difficulty === 'medium' ? totalFrags * 20 :
                   difficulty === 'hard' ? totalFrags * 25 : totalFrags * 22;
...
function generateAllLevels() {
  const levels = [];
  const shuffledIdioms = shuffle(idiomPool);

  let idiomIdx = 0;

  function nextIdioms(n) {
    const result = [];
    for (let i = 0; i < n; i++) {
      result.push(shuffledIdioms[idiomIdx % shuffledIdioms.length]);
      idiomIdx++;
    }
    return result;
  }

  // Levels 1-15: Easy — 2 rows × 4 cols, 2 idioms
  for (let i = 0; i < 15; i++) {
    levels.push(buildLevel(i + 1, 'easy', nextIdioms(2), 2, 0, 4));
  }

  // Levels 16-30: Easy — 3 rows × 4 cols, 3 idioms
  for (let i = 0; i < 15; i++) {
    levels.push(buildLevel(i + 16, 'easy', nextIdioms(3), 3, 0, 4));
  }

  // Levels 31-50: Medium — 4 rows × 4 cols, 4 idioms
  for (let i = 0; i < 20; i++) {
    levels.push(buildLevel(i + 31, 'medium', nextIdioms(4), 4, 0, 4));
  }

  // Levels 51-70: Medium — 5 rows × 4 cols, 5 idioms
  for (let i = 0; i < 20; i++) {
    levels.push(buildLevel(i + 51, 'medium', nextIdioms(5), 5, 0, 4));
  }

  // Levels 71-85: Hard — 5 rows × 5 cols, 5 idioms (4 horizontal + 1 vertical in gap column)
  for (let i = 0; i < 15; i++) {
    levels.push(buildLevel(i + 71, 'hard', nextIdioms(5), 4, 1, 5));
  }

  // Levels 86-95: Hard — 6 rows × 5 cols, 6 idioms (5 horizontal + 1 vertical in gap column)
  for (let i = 0; i < 10; i++) {
    levels.push(buildLevel(i + 86, 'hard', nextIdioms(6), 5, 1, 5));
  }

  // Levels 96-100: Hard — 6 rows × 5 cols, 6 idioms (4 horizontal + 2 vertical, two gap columns)
  for (let i = 0; i < 5; i++) {
    levels.push(buildLevel(i + 96, 'hard', nextIdioms(6), 4, 2, 6));
  }

  return levels;
}

// Generate and write seed file
const levels = generateAllLevels();

const seedContent = `const { sequelize, LevelConfig } = require('../models');
...
async function seed() {
  ...
  console.log('Seed data inserted: ' + levels.length + ' levels.');
  process.exit(0);
}
...
fs.writeFileSync(path.join(__dirname, 'seedLevels.js'), seedContent);
console.log('Generated ' + levels.length + ' levels in seedLevels.js');
